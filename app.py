...
def analysis():
    ...
    fig1 = px.bar(df,x=month.index.values,y=month.values,color=month.index.values,title='Ransomware Attack According to Months',labels={'x':'Months','y':'Ransomware Attacks'})
    ...
    fig4=px.scatter(df,y='Top ransomware attacks',x='Revenue',color='Month',title='Ransomware Attack According to Revenue')
...
def analysis_1():
    ...
    fig5 = px.bar(df,x=rev.index.values,y=rev.values,color=rev.index.values,title='Revenue',labels={'y':'Ransomware Attacks','x':'Revenue(In Million)'})
    ...
    fig7 = px.scatter(df,y='Organisation Size',x='Top ransomware attacks',color='Organisation Size',title='Ransomware Attack According to Organisation Size')
    graph7=json.dumps(fig7,cls=plotly.utils.PlotlyJSONEncoder)

    fig8 = px.bar(df,y='Organisation Size',x='Top ransomware attacks',color='Organisation Size',title='Ransomware Attack According to Organisation Size')
...
def analysis_3():
    ...
    fig11 = px.scatter(df,y='YEAR',x='Top ransomware attacks',color='YEAR',title='Ransomware Attack Yearwise Representation')
    graph11=json.dumps(fig11,cls=plotly.utils.PlotlyJSONEncoder)

    sec = df['Sector'].value_counts()
    fig12 = px.bar(df,y= sec.index.values,x=sec.values,color=sec,title='Sectors Attacked',labels={'y':'Sectors','x':'Ransomware Attacks'})
